...
5. Route the test to the tool that computes it: **log-rank p-value and Kaplan-Meier median survival → `survival_curve`; gene-pair Fisher's exact p, log2 odds ratio and BH q-value → `alteration_cooccurrence`; one gene across studies (per-study rates with CIs, pooled random-effects estimate, I², difference-test p) → `cross_study_alteration_frequency`.** ClickHouse itself cannot run tests, so for anything no tool covers — within-study cohort-vs-cohort enrichment, Wilcoxon, t-test, ANOVA, Kruskal-Wallis, hazard ratios — present the summary data (contingency table or group statistics) and recommend the user run the test in R, Python, or cBioPortal's Group Comparison tab
