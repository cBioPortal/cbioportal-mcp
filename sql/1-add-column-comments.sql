...
ALTER TABLE clinical_data_derived MODIFY COLUMN attribute_name
  COMMENT 'Clinical attribute name (e.g., SAMPLE_TYPE, CANCER_TYPE, AGE, OS_MONTHS). Use with attribute_value for filtering. AGE is age at diagnosis but may be floored or capped for de-identification: every *_target_gdc study records all patients under 18 as 18. For age statistics check how many patients sit at the min/max first, and compute age from DAYS_TO_BIRTH (-days / 365.25) when AGE is floored — see clinical-data-guide "Age statistics".';
