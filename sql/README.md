...
| 6 | `6-cross-study-views.sql` | `gene_alteration_counts_per_study(studies, gene, alteration, oncotree_codes)` — per-study cohort / profiled / altered counts with the panel ∪ WES denominator, raw-SQL parity for the `cross_study_alteration_frequency` tool (which does not depend on it). See `cbioportal://mutation-frequency-guide` → "Across named studies". |
